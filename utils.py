...
def get_scheduler(config, optimizer):
    assert config.sch in ['StepLR', 'MultiStepLR', 'ExponentialLR', 'CosineAnnealingLR', 'ReduceLROnPlateau',
                        'CosineAnnealingWarmRestarts', 'WP_MultiStepLR', 'WP_CosineLR', 'NO_LR'], 'Unsupported scheduler!'
...
class BCELoss(nn.Module):
    ...
    def forward(self, pred, target):
        size = pred.size(0)
        bce_loss = F.binary_cross_entropy_with_logits(pred.squeeze(1), target, reduction='mean')

        return bce_loss / size
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        smooth = 1
        pred = F.sigmoid(pred)  # 网络最后没有sigmoid，这里使用sigmoid得到概率值
...
def cal_params_flops(model, size, logger):
    ...
    logger.info(f'flops: {flops/1e9:.2f}, params: {params/1e6:.2f}, Total params: : {total/1e6:.2f}')
...
if __name__ == '__main__':
    ...
    pred = torch.tensor([[0.8, 0.2],
                         [0.3, 0.4]], dtype=torch.float)
    target = torch.tensor([[1, 0],
                           [0, 0]], dtype=torch.float)
    bce_loss = BCELoss()
    print("bce_loss: ", bce_loss(pred, target))

    """dice loss check"""
    # pred = torch.tensor([[0.5322, 0.4932, 0.1764],
    #                      [0.3107, 0.5297, 0.1604],
    #                      [0.3841, 0.3537, 0.3574],
    #                      [0.3323, 0.8301, 0.6436]], dtype=torch.float)
    ...
    # pred = F.sigmoid(pred)
    dice_loss = DiceLoss()
    print("dice_loss: ", dice_loss(pred, target))
    ...
    print("bceDice_loss: ", criterion(pred, target))
    ...
    # 合并 BCE 损失和 Dice 损失
    bceDice_loss = bce_loss + dice_loss

    print(f"BCE Loss: {bce_loss.item()}, Dice Loss: {dice_loss.item()}, BCE+Dice Loss: {bceDice_loss.item()}")
